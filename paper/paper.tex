...
The literature review is organised through the project files \texttt{literature/search\_protocol.md}, \texttt{literature/source\_matrix.csv} and \texttt{literature/notes/}. The review combines official documentation, maintainer repositories, academic papers and supporting community sources. These sources are grouped thematically instead of being summarised one by one.
...
The experimental design is documented in \texttt{docs/experimental\_design.md}. The notebook is \texttt{notebook/compatibility\_barrier\_experiment.ipynb}.
...
The raw dataset is stored in \texttt{data/raw/synthetic\_trials.csv}. The processed strategy-level results are stored in \texttt{data/processed/aggregated\_results.csv}. The dataset contains 600 rows. Each strategy is evaluated through 120 synthetic trials.

\subsection{Aggregated Results by Strategy}

Table~\ref{tab:aggregated-results} reports the current strategy-level results.
...
The notebook generates five figures. The most important figures for the final paper are likely \texttt{strategy\_comparison.png}, \texttt{dependency\_risk\_heatmap.png} and \texttt{setup\_effort\_boxplot.png}.
...
\subsection{Proton as Proof of Engineering Feasibility}
